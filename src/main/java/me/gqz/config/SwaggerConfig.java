...
public class SwaggerConfig {
    ...
    public Docket reservationApi() {
        return new Docket(DocumentationType.SWAGGER_2).
                apiInfo(apiInfo())
                .select()
                .apis(RequestHandlerSelectors.basePackage("me.gqz.restful"))
                .paths(PathSelectors.any())
                .build();
    }

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("GQZ | GQZ-admin")
		        .description("The API for GQZ admin.").version("1.0").build();
	}
